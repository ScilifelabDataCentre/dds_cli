...
@click.pass_obj
def add_user(_, username, email, role):
...
@click.pass_obj
def create(_, username, title, description, principal_investigator, is_sensitive):
    """Create a project."""
    try:
        with dds_cli.project_creator.ProjectCreator(
            username=username,
        ) as creator:
            created, project_id, _ = creator.create_project(
                title=title,
